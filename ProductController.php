...
class ProductController extends Controller
{
  ...
  $sleeveArray = array('Full sleeve','Half sleeve','Short sleeve','Sleevless');

      return view('admin.products.add_product',compact('categories_dropdown','sleeveArray'))->with('flash_message_success',' Product Add Successfully');
  ...
  public function edit(Request $request,$id=null)
  {
   ...
   if (empty($data['care'])) {
    $care ='';
      ...
      'care'=>$data['care'],'sleeve'=>$data['sleeve'],'price'=>$data['price'],'image'=>$filename]);
...
$productsAll = $productsAll->whereIn('products.leeve',$sleeveArray);
}
if (!empty($_GET['size'])) {
$sizeArray = explode('-',$_GET['size']);
$productsAll = $productsAll->join('product_attributes','product_attributes.product_id','=','product_id')
...
// echo "<pre>";print_r($productsAll);die();
 ...
 $sizeArray = ProductAttribute::select('size')->where('size','!=','')->groupBy('size')->get();
 $sizeArray = Arr::flatten(json_decode(json_encode($sizeArray),true));
 // echo "<pre>";print_r($sizeArray);die();
  return view('products.listing',compact('categories','categoryDetails',
  'productsAll','url','colorArray','sleeveArray','sizeArray','breadcrumb'));
}
...
if ($request->ajax()) {
  $data = $request->all();
  echo "<pre>";print_r($data);die();
  $getProductPrice = ProductAttribute::where(['product_id'=>$data['product_id'],'size'=>$data['size']]);
...
elseif($data['payment_method']=="Debit-credit"){
  return redirect('stripe');
    ...
    return redirect()->route('laravel-bitpay.webhook.capture');
 ...
 public function DebitCreditcard(Request $request)
 {
   $user_email = Auth::user()->email;
   DB::table('carts')->where('user_email',$user_email)->delete();

  return view('orders.Debit-Credit-card');
 }
 ...
 public function square(Request $request)
 {
  ...
  return view('orders.square');
 }
 public function tranwise(Request $request)
 {
   $user_email = Auth::user()->email;
   DB::table('carts')->where('user_email',$user_email)->delete();

  return view('orders.tranwise');
 }
